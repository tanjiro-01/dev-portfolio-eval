      ...
      {!repos.length ? (
        ...
        <ul className="grid grid-cols-1 @md:grid-cols-2 gap-4">
          {repos.map((repo) => (
            <li
              key={repo.url}
              className="flex flex-col bg-slate-800/40 border border-slate-700/50 rounded-xl p-5 hover:bg-slate-800 hover:border-slate-600 transition duration-300 shadow-sm relative group overflow-hidden"
            >
              <div className="absolute top-0 left-0 w-1 h-full bg-slate-700/50 group-hover:bg-amber-500 transition duration-300"></div>
              <a
                href={repo.url}
                target="_blank"
                rel="noreferrer"
                className="text-amber-300 hover:text-amber-200 font-semibold text-[15px] hover:underline flex items-center gap-2"
              >
                <span>{repo.name}</span>
                <svg
              ...
              <p className="text-sm text-slate-400 mt-2 mb-4 line-clamp-2 leading-relaxed flex-1">
