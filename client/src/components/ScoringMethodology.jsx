    ...
    <section className="@container bg-slate-900/80 backdrop-blur-md border border-slate-800 rounded-2xl p-6 shadow-xl">
      ...
      <div className="grid grid-cols-1 @sm:grid-cols-2 @lg:grid-cols-3 gap-4">
        {scoringRules.map((rule) => (
          <article
            key={rule.title}
            className="p-4 bg-slate-800/40 border border-slate-700/50 rounded-xl hover:bg-slate-800 transition-colors"
          >
            <strong className="block text-slate-200 text-sm mb-1">
              {rule.title}
            </strong>
