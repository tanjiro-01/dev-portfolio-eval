...
const SearchForm = ({ defaultValue = "", onSubmit, loading }) => {
  ...
  return (
    <form className="w-full grid grid-cols-1 sm:grid-cols-[1fr_auto] gap-3" onSubmit={handleSubmit}>
      <input
      ...
      />
      <button 
        className="flex items-center justify-center gap-2 bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-500 hover:to-indigo-500 text-white font-semibold rounded-xl px-6 py-3 transition-all hover:scale-[1.02] active:scale-95 shadow-lg shadow-blue-500/25 disabled:opacity-50 disabled:pointer-events-none border border-blue-500/50" 
        type="submit" 
        disabled={loading}
      >
        {loading ? (
