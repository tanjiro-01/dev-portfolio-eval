...
const Home = () => {
  ...
  return (
    <main className="min-h-screen flex flex-col items-center justify-center p-4 sm:p-8 bg-slate-950 relative overflow-hidden">
      {/* Background Glow */}
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[600px] h-[600px] bg-amber-700/15 rounded-full blur-[120px] pointer-events-none"></div>
      ...
      <div className="w-full max-w-3xl relative z-10 flex flex-col gap-10">
        ...
        <header className="text-center">
          ...
          <p className="text-slate-400 text-lg sm:text-xl max-w-2xl mx-auto leading-relaxed">
            Enter a username to generate a comprehensive report with activity,
            quality, diversity, community, and hiring-readiness scores.
          </p>
        </header>
        ...
        <section className="bg-slate-900/30 backdrop-blur-md border border-slate-800/60 rounded-3xl p-6 sm:p-8 shadow-xl text-center w-full max-w-2xl mx-auto relative group">
          <div className="absolute -inset-0.5 bg-gradient-to-r from-amber-500/10 to-stone-500/10 rounded-3xl blur opacity-0 group-hover:opacity-100 transition duration-1000"></div>
          <div className="relative z-10">
            ...
            <div className="flex flex-col sm:flex-row gap-3 justify-center mb-6">
              <input
                className="bg-slate-900 border border-slate-700 text-slate-200 rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-amber-500 flex-1 max-w-[300px]"
              ...
              <div className="flex flex-wrap justify-center gap-2 mb-8">
                {compareUsers.map((username) => (
                  <button
                    key={username}
                    type="button"
                    className="flex items-center gap-2 bg-amber-500/10 hover:bg-rose-500/20 text-amber-300 hover:text-rose-300 border border-amber-500/20 hover:border-rose-500/30 rounded-full px-4 py-1.5 text-sm font-medium transition-colors"
            ...
            <div className="pt-6 border-t border-slate-800/50">
              <button
                className="text-slate-400 hover:text-amber-300 text-sm font-medium transition-colors"
                ...
                <div className="flex flex-wrap gap-2">
                  {savedPresets.map((preset) => (
                    <button
                      key={preset.id}
                      type="button"
                      className="bg-slate-800 border border-slate-700 hover:border-amber-500 hover:text-amber-200 text-slate-300 rounded-lg px-4 py-2 text-sm transition-colors"
                      onClick={() =>
